package com.weilaios.iqxceqhnhubt.da.service;

import com.weilaios.iqxceqhnhubt.model.UvsvSEmployeeTest;
import com.weilaios.iqxceqhnhubt.utils.Result;
import java.util.*;
import com.baomidou.mybatisplus.extension.service.IService;


/**
 * uvsvSEmployeeTest 服务接口
 *
 * @author 阿旺
 * @date Apr 7, 2022 3:06:40 PM
 * @since v0.1
 */
public interface UvsvSEmployeeTestService extends IService<UvsvSEmployeeTest> {

      

	Result Activity_1nv16yb(Map<String,Object> paramMap) throws Exception;
}

