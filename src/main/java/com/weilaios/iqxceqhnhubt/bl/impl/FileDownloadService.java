...
public class FileDownloadService {
    ...
    public JSONObject processMainExecute(String paramsJson) throws BusinessException {
        ...
        paramsJson = StartEvent_1wj0yk1p62416938(paramsJson);
        //获取流程返回的结果
    ...
    public String StartEvent_1wj0yk1p62416938(String paramsJson) throws BusinessException {
        log.info("========执行开始方法=======");
        String taskName = "开始事件";
        String processName = "文件下载";
        String errorSuffix = "[" + processName + "-" + taskName + "]";
        try{
            paramsJson = Activity_1f93dbi(paramsJson);
            return paramsJson;
    ...
    public String Activity_0ih3b8x(String paramsJson) throws BusinessException {
        log.info("========执行downloadFile方法=======");
        String taskName = "文件下载";
        String processName = "文件下载";
        String errorSuffix = "[" + processName + "-" + taskName + "]";
        try {
            DocumentContext paramsJsonContext = JsonPath.parse(paramsJson);
            Object downFileValue = null;
            String downFileName = "";
            //判断下载的内容
            ...
            if (downFileValue instanceof List) {
                ...
                fileList = (List<Map>) downFileValue;
            } else if (downFileValue instanceof Map) {
                fileList.add((Map) downFileValue);
            } else {
                //如果不是数组格式，则认为是文件地址
            ...
            downFileMap.put("downFileFrom", Constants.DOWN_FILE_FROM_TYPE_FILE);
            downFileMap.put("fileName", downFileName);
            downFileMap.put("dataList", fileList);
            String resultKey ="Activity_0ih3b8x";
            ...
            log.error("处理失败", e);
            throw new BusinessException("文件下载处理失败" + errorSuffix, e);
        }
    }
    public String Activity_1f93dbi(String paramsJson) throws BusinessException {
        log.info("========执行数据查询方法=======");
        ...
        try {
            ...
            //获取返回数据存在的key值
            String resultKey = "Activity_1f93dbi";
            //处理请求参数
            ...
            String returnStr = JSON.toJSONString(uvsvSUserService.Activity_1f93dbi(paramMap));
            ...
            paramsJson = Activity_0ih3b8x(paramsJson);
            return paramsJson;
        } catch (BusinessException e) {
            log.error("异常", e);
            throw e;
