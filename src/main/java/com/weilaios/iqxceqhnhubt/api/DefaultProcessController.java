...
public class DefaultProcessController {
    ...
    * @return
    */
    @PostMapping(value = "/v1/p6238200de4b047fabe30d90b")
    public Result p6238200de4b047fabe30d90bDo(HttpServletRequest request, HttpServletResponse response, @RequestBody JSONObject params) {
        log.info("===============流程执行开始=======");
        Result result = new Result();
        try {
            JSONObject paramsJSONObject = params.getJSONObject("paramsJson");
            String processName = "默认流程1";
            ...
            //调用执行流程方法
            JSONObject returnDataJson = defaultProcessService.processMainExecute(dataJson);
            //移除过程执行中的临时变量
